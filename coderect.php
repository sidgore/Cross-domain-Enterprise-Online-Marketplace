  ...
  <script>
                        $(document).ready(function () {
                            $("#demo3 .stars").click(function () {
                                ...
                                $.post('rating.php',{rate:$(this).val(),product: 'coderect'},function(d){
                                    if(d>0)
                                    {
  ...
  <section>
    ...
    <div class="col-lg-2"></div>
     ...
     <p>Hello <?php echo $login_session; ?>, you have seen this page <?php echo $_SESSION['count']; ?> times.
      ...
      <p>
      <h2><code>Code Rectifier<span class="glyphicon glyphicon-fire"></span></h2></code> </p><br><p>
       This cool app picks up data regarding one's coding errors from the IDE ,puts them in <kbd>HDFS</kbd>, runs <kbd>PIG scripts</kbd> on it and then draws graph from the pattern. So how is it helpful for you?
      For instance - you have a company with 10 coders who have unique  style of coding, each one of them face different kind of issues while coding, for example - "A" usually faces problems like Array Index out of bound, "B" faces something else. We find out pattern of these issues and help the employer train employees according the needs, so in real time development coders perform better. Might sound complex but all those patterns are graphed using <kbd>Chart.js</kbd> and it gives you good looking visualization in the form of graphs
      </p>  <br>
      </p>  </h2></p></p></blockquote>
...
<div style='clear:both;'></div>
                ...
                <form action ="comment.php?product=coderect" method="POST">
...
$sql = "SELECT c.*, t.* FROM comment c, tbl_rating t where c.user = t.user_id and c.product ='coderect' and t.product ='coderect'";
...
$sql1 = "SELECT user FROM products_visited  where product ='coderect' and user = '".$login_session."'" ;
 $sql2 = "INSERT INTO `products_visited` ( `user`, `product`, `counts` ) VALUES ('" . $login_session . "', 'coderect',  1); ";
 $sql3= "UPDATE `products_visited` SET counts=counts+1 where product='coderect' and user = '".$login_session."'" ;    
